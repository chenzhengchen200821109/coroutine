...
struct schedule {
	...
	ucontext_t main;        // 正在running的协程在执行完后需切换到的上下文，
                            //由于是非对称协程，所以该上下文用来接管协程结束后的程序控制权
...
coroutine_new(struct schedule *S, coroutine_func func, void *ud) {
	...
	if (S->nco >= S->cap) { // 调整调度器S中协程的最大容量,
                            //然后将协程任务co加入调度器S,并返回该协程任务的id
		int id = S->cap;
		...
		++S->nco;
		return id;
	} else { // 将协程任务co加入调度器S,并返回该协程任务的id
		int i;
		for (i=0;i<S->cap;i++) {
...
coroutine_resume(struct schedule * S, int id) {
	...
	case COROUTINE_READY:
         ...
         * from a call to getcontext(3)). Before invoking makecontext(), the caller must allocate
         * a new stack for this context and assign its address to ucp->uc_stack, and define a 
